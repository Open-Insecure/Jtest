...
//        System.out.println("....."+parseCaopron(caoPronIndexUrl+"/video/51cbc9a0f9caeabd6f05/岛崎-絵理子-eriko-shimazaki01"));
     ...
     * @return
     */
    public static String parse91pron(String targetUrl) throws IOException, CloneNotSupportedException {
     ...
     * @return
     */
    public static String parseCaopron(String targetUrl) throws NoSuchAlgorithmException, KeyManagementException, IOException, CloneNotSupportedException, DocumentException {
        ProxyBean proxy= JdbcUtil.getProxy();
        System.out.println("start connet to proxy:["+proxy.toString()+"]");
        //使用新的访问代理的方法
         try{crawlerUtil.clientByProxyCreate("http", proxy.getIp(), "http://" + proxy.getIp());
